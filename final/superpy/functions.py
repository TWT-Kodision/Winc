...
class Functions: 
    ...
    def convert_to_date_object(self, date):
        date_object = datetime.strptime(date, '%Y-%m-%d').date()
        return date_object

    #using begin and end date, it makes an array with al days in between. 
    def make_date_array(self, start_date, end_date):
        date_array = []
    ...
    #makes new csv file 
    def make_new_csv_file(self, filename, column_name=None):
        try:
    ...
    def get_expired_product_list(self, day = date.today()):
        inventory_list = self.get_inventory_list()
        del inventory_list[0] #remove header
        expired_products = []
        for product in inventory_list:
            date_object = self.convert_to_date_object(product[4])
            if (date_object) <= day:
                expired_products.append(product)
        return expired_products

    #get sold list
    def get_sold_product_list(self):
    ...
    def get_products_date_range(self, begin_date, end_date, bought_or_sold):
        begin_date 
        end_date 
        list_range = []
        ...
        for product in product_list:
            product_date_object = self.convert_to_date_object(product[2])
    ...
    def calculate_profit(self, begin_date, end_date):
        #make sold list for specified range
        sold_list_range = self.get_products_date_range(begin_date, end_date, "sold")
        profit = 0
        for product in sold_list_range:
            product_bought_details = self.find_product_by_id(product[1])
    ...
    def count_each_product_inventory(self):
        inventory_list = self.get_inventory_list()
        counts = dict()
    ...
    def count_sold_products_per_day(self, date):
        sold_products = self.get_sold_product_list()
        count = 0
    ...
    def make_dict_day_count(self, start_date, end_date):
        dates = self.make_date_array(start_date, end_date)
        sold_on_dates = {}
        for date in dates:
            sold_on_dates[date] = self.count_sold_products_per_day(str(date))
        return sold_on_dates

    #create graph of number of sold products per day
    def make_date_sold_graph(self, start_date, end_date):
        dates_sold = self. make_dict_day_count(start_date, end_date)
        self.create_graph(dates_sold)
